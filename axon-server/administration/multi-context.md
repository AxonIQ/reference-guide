...
More on replication groups can be found in the [clustering](clustering.md) section.
...
| event.index-format | JUMP_SKIP_INDEX | Index type used for the events in this context (JUMP_SKIP_INDEX or BLOOM_FILTER_INDEX |
...
| snapshot.index-format | JUMP_SKIP_INDEX | Index type used for the snapshots in this context (JUMP_SKIP_INDEX or BLOOM_FILTER_INDEX |
...
for all contexts that are created from this version onwards. This index format uses a global index to locate the last event for a specific aggregate, 
...
> 
>  Note that when you delete an existing context with the preserve data option and then recreate it, without specifying the index format, 
