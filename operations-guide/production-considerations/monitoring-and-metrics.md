 ...
 * An optional `Long` through the  defining the current position of the `TrackingToken`.
 * An optional `Long` defining the position at reset of the `TrackingToken`. 
 ...
 * An optional `Long` defining the position on the `TrackingToken` when merging will be completed. 
