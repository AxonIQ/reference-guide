...
2. `EventProcessorLatencyMonitor` - Measures the difference between an event's timestamp and the current time, showing how far behind an [event processor](events/event-processors/README.md) is. Note that [triggering a reset](events/event-processors/streaming.md#replaying-events) will impact this metric!
